...
DEFAULT_OUTPUT = Path("/srv/samba/share") / "zhihu_profile_today_updates_min_test.json"
DEFAULT_LIMIT = 30
        ...
        items.append(
            {
                ...
                "summary": html_to_text(content_html) if content_html else str(excerpt),
        ...
        items.append(
            {
                ...
                "summary": html_to_text(content_html) if content_html else excerpt,
                "author_name": str(author.get("name") or ""),
                "voteup_count": item.get("voteup_count"),
                "comment_count": item.get("comment_count"),
                "source_mode": "api",
        ...
        items.append(
            {
                ...
                "title": str(item.get("excerpt_title") or ""),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="最小测试：抓取知乎个人主页今日更新（回答/文章/想法，多接口混合，DOM 兜底）")
    ...
    try:
        ...
        payload = {
            ...
            "dom_today_count": len(dom_today_items),
            "dom_only_count": len(dom_only_items),
